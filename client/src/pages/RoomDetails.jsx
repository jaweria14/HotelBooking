...
export default function RoomDetails() {
    ...
    const checkAvailability = async () => {
        try {
            //if checkin data is > than check out date 
            if (checkInDate >= checkOutDate) {
                toast.error('CheckIn date must be lesser than CheckOut date');
                return;
            }
                ...
                <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                    {room?.images.length > 1 && room.images.map((image, index) => (
                        <img
                            ...
                            src={image}
                            alt="Room Image"
                            className={`w-full h-[215px] rounded-xl shadow-md object-cover cursor-pointer ${mainImage === image && 'outline-3 outline-orange-500'}`}
                        />
                    ))}
                </div>
            </div>
            ...
            <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
                ...
                <div className='flex flex-col flex-wrap md:flex-row items-start md:items-center gap-4 md:gap-10 text-gray-500'>
                    ...
                    <div className="flex flex-col">
                        ...
                        <input onChange={(e) => setCheckInDate(e.target.value)} min={new Date().toISOString().split('T')[0]} type="date" id='checkInDate' placeholder='Check-In' className='w-full rounded border border-gray-300 px-3 py-2 mt-1.5 outlinne-none ' required />
                    </div>
                    {/* verticle line */}
                    ...
                    <div className="flex flex-col">
                        ...
                        <input onChange={(e) => setCheckOutDate(e.target.value)} min={checkInDate} disabled={!checkInDate} type="date" id='checkOutDate' placeholder='Check-In' className='w-full rounded border border-gray-300 px-3 py-2 mt-1.5 outlinne-none ' required />
                    </div>
                    {/* verticle line*/}
                    <div className='hidden md:block w-px h-12 bg-gray-300/70'></div>
                    {/* third div  */}
                    <div className="flex flex-col">
                        ...
                        <input onChange={(e) => setGuests(e.target.value)} value={guests} type="number" id='guest' placeholder='1' className='max-w-20 rounded border border-gray-300 px-3 py-2 mt-1.5 ' required />
                    </div>
